...
const CardGrid = ({menuData, setActiveSubItem, t, isHe}) => {
    ...
    const handleItemClick = (sub) => {
        if (sub?.contentMode === 'contactus') {
            // 1. Target the contact form container by its element ID
            ...
            if (targetElement) {
                ...
                // Fallback to top if the contactus element is missing from the DOM
                window.scrollTo({top: 0, behavior: 'smooth'});
            }
        } else {
            // 2. Default fallback behavior for 'editor' or 'linker' modes
