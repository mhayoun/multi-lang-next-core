...
const GalleryBanderole = ({media, isHe}) => {
    ...
    const renderMedia = (item, index, isSingle = false) => {
        ...
        if (isSingle) {
            if (isYoutube) {
                return (
                    <div
                        className="w-full aspect-video rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-100">
                        <iframe
            ...
            >
                {isYoutube ? (
                    <div className="w-full h-full relative">
                        <img src={yt.thumbnail} className="absolute inset-0 w-full h-full object-cover opacity-80"
                             alt=""/>
                        <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                            <div
                                className="bg-red-600 p-3 rounded-2xl shadow-xl group-hover:scale-110 transition-transform">
    ...
    return (
        <div className="mb-12" dir={isHe ? 'rtl' : 'ltr'}>
            ...
            {selectedIndex !== null && (
                <div
                    className="fixed inset-0 z-[100000] bg-slate-950/98 backdrop-blur-3xl flex items-center justify-center animate-in fade-in duration-300"
                    onClick={closeLightbox}
                >
                    {/* FIXED CLOSE BUTTON - Positioned relative to screen window to prevent clipping */}
                    <div className="fixed top-6 right-6 z-[100001]">
                        <button
                            type="button"
                            className="bg-black/60 hover:bg-black/80 text-white p-3 rounded-full backdrop-blur-xl border border-white/20 shadow-2xl transition-transform active:scale-90"
                            onClick={(e) => {
                                e.stopPropagation();
                                closeLightbox();
                            }}
                    ...
                    <button
                        type="button"
                        className="absolute left-4 md:left-8 text-white/50 hover:text-white p-4 transition-all z-[100000]"
                        onClick={(e) => {
                            e.stopPropagation();
                            isHe ? nextItem() : prevItem();
                        }}
                    >
                        <ChevronLeft size={48}/>
                    </button>
                    ...
                    <button
                        type="button"
                        className="absolute right-4 md:right-8 text-white/50 hover:text-white p-4 transition-all z-[100000]"
                        onClick={(e) => {
                            e.stopPropagation();
                            isHe ? prevItem() : nextItem();
                        }}
                    >
                        ...
                        <div className="relative w-full flex justify-center items-center h-[70vh]">
                            {(() => {
                                ...
                                if (checkIsYoutube(currentItem)) {
                                    return <iframe src={getYoutubeInfo(currentItem.url).embed}
                                                   className="w-full h-full rounded-xl shadow-2xl aspect-video"
                                                   allow="autoplay; encrypted-media" allowFullScreen title="Youtube"/>;
                                } else if (currentItem.type === 'video') {
                                    return <video src={currentItem.url} controls autoPlay
                                                  className="max-w-full max-h-full rounded-xl shadow-2xl"/>;
                                } else {
                                    return <img src={currentItem.url}
