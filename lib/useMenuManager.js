...
export const useMenuManager = () => {
    ...
    }, [menuData, newsData, logo, footerData, siteSettings, homeData, mounted]);
    ...
    const removeFile = async (targetId, subId, type, index, isNews = false) => {
        ...
        // 1. Attempt the Cloud Deletion (Vercel Blob)
        try {
            ...
            if (!response.ok) {
                // We throw here to jump to the catch block for logging,
                // but the UI will still update below.
                throw new Error(result.error || 'Server failed to delete blob');
            }
            console.log("✅ Vercel Blob deleted successfully");
        } catch (error) {
            // Log the error so you know the cloud storage is out of sync
            console.warn("⚠️ Cloud deletion failed, but removing from local state anyway:", error.message);
        }

        // 2. Update the Local UI State (Moved outside the try block)
        // This ensures that even if the API fails (Access Denied), the item vanishes from the user's screen.
        setter(prev => prev.map(item => {
            if (item.id !== targetId) return item;
